...
import (
	...
	v1 "sigs.k8s.io/kube-scheduler-wasm-extension/kubernetes/proto/api"
)
...
func filter(input *FilterInput) api.Status {
	pattern, ok := input.Pod.GetMetadata().GetAnnotations()[regexAnnotationKey]
	if !ok {
	...
	nodeName := input.NodeInfo.Node.GetMetadata().GetName()
