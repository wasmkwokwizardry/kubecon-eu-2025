...
var plugins = map[string]string{
	"Go":     "../guest-go/plugin.go.wasm",
	"TinyGo": "../guest-go/plugin.tinygo.wasm",
	"Rust":   "../guest-rust/target/wasm32-unknown-unknown/release/filter_vtproto_guest_rust.wasm",
}
				...
				input := &filter.FilterInput{
					Pod: &v1.Pod{
						Metadata: &metav1.ObjectMeta{
							Name: proto.String(fmt.Sprintf("pod-%d", i)),
					...
					NodeInfo: &filter.NodeInfo{
						Node: &v1.Node{
							Metadata: &metav1.ObjectMeta{
								Name: proto.String(fmt.Sprintf("kubecon-%d", i)),
							},
						},
					},
				}
